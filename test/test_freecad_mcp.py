...
SCRIPT = REPO_ROOT / "tools" / "run_freecad_mcp.sh"
...
class FreeCadMcpTests(unittest.TestCase):
    def test_server_supports_initialize_and_tools_list(self):
        ...
        try:
            ...
            self.assertEqual(initialize["result"]["serverInfo"]["name"], "freecad")
            ...
            self.assertEqual(
                tool_names,
                {"get_runtime_info", "create_document", "export_document", "run_python_script"},
            )
        finally:
            proc.terminate()
            proc.wait(timeout=5)
